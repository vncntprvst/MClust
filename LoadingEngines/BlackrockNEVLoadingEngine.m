...
%       - Modified to work with the latest version of the MClust v4.4
...
if ~exist('recordUnits', 'var')
    ...
    if numel(readElectrodes)<=4 % original code for tetrode data
        ...
        for idx = 1:length(readElectrodes)
            elIdx= NEV.Data.Spikes.Electrode == readElectrodes(idx);
            waveForms(:,idx,:) = NEV.Data.Spikes.Waveform(1:32,elIdx)';
        end
    else
